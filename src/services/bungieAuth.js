...
const BUNGIE_CONFIG = {
  ...
  scopes:
    "ReadBasicUserProfile ReadCharacterData ReadInventoryData ReadClanData ReadRecords",
  ...
  async post(url, data, options = {}) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Network error: Unable to connect to ${fullUrl}. Check CORS configuration.`,
        );
      }
      throw fetchError;
    }
...
export class BungieAuthService {
  ...
  static initiateOAuth() {
    try {
      ...
      // --- BEGIN ADDED LOGGING ---
      console.log(
        "Redirecting to Bungie for authorization:",
        authURL.toString(),
      );
      // --- END ADDED LOGGING ---
  ...
  static recoverFromStateValidationError() {
    try {
      ...
      import("../utils/errorHandler.js").then(
        ({ cleanupOAuthState, cleanupExpiredOAuthStates }) => {
          ...
          logger.info(
            "OAuth state recovery completed - ready for fresh authentication",
          );
        },
      );
      ...
      logger.error("Failed to recover from OAuth state error", {
        error: error.message,
      });
      return {
        success: false,
  ...
  static testOAuthStateValidation() {
    ...
    const results = {
      ...
      allStorageWorking:
        retrievedLocal === testState &&
        retrievedSession === testState &&
        retrievedCookie === testState,
    };
  ...
  static async exchangeCodeForTokens(code) {
    ...
    if (!code || typeof code !== "string") {
      throw new Error(
        "Invalid authorization code: code must be a non-empty string",
      );
    }
    ...
    if (trimmedCode.length < 10 || trimmedCode.length > 500) {
      throw new Error(
        "Invalid authorization code: code length is outside expected range",
      );
    }
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Invalid token response: missing or invalid refresh_token",
        );
      }
      ...
      if (error.message.includes("401") || error.status === 401) {
        logger.error(
          "Token exchange failed with 401 Unauthorized",
          errorContext,
        );
  ...
  static async getBungieConnection() {
    try {
      ...
      const decryptedData = {
        ...data,
        access_token: isTokenEncrypted(data.access_token)
          ? decryptToken(data.access_token)
          : data.access_token,
        refresh_token: isTokenEncrypted(data.refresh_token)
      ...
      console.error(
        "Error fetching Bungie connection from localStorage:",
        error,
      );
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            typeof decryptedRefreshToken !== "string"
          ) {
            throw new Error("Invalid refresh token format after decryption");
          }
        } catch (decryptError) {
          logger.error(
            "Token refresh failed: Unable to decrypt refresh token",
            {
        ...
        if (
          ...
          throw new Error(
            "Invalid refresh response: missing or invalid access_token",
          );
        }
        ...
        if (retryCount <= maxRetries) {
          ...
          logger.warn(
            `Token refresh failed, retrying in ${delay}ms`,
            errorContext,
          );
  ...
  static async getCharacters(membershipType, membershipId) {
    ...
    try {
      const response = await apiClient.get(
        `/destiny2/profile/${membershipType}/${membershipId}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
